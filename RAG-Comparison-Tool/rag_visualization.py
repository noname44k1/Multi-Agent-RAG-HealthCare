...
"""
Script để trực quan hóa kết quả so sánh các phương pháp RAG
"""
...
def visualize_results(processed_data, output_dir="visualization"):
    ...
    bars = plt.bar(methods, avg_accuracy, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    ...
    bars = plt.bar(methods, avg_relevance, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    ...
    bars = plt.bar(methods, avg_context, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    ...
    bars = plt.bar(methods, avg_times, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    ...
    bars = plt.bar(methods, avg_tokens, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    ...
    bars = plt.bar(methods, avg_docs, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    ...
    for i, method in enumerate(methods):
        ...
        plt.bar(x + (i - 1.5) * width, times, width, label=method)
    ...
    for i, method in enumerate(methods):
        ...
        plt.bar(x + (i - 1.5) * width, tokens, width, label=method)
    ...
    # 11. Biểu đồ heatmap tổng hợp
    plt.figure(figsize=(12, 8))
    
    # Chuẩn bị dữ liệu cho heatmap
    metrics = ['Độ chính xác', 'Độ liên quan', 'Độ chính xác ngữ cảnh', 'Thời gian (s)', 'Token']
    
    heatmap_data = np.zeros((len(metrics), len(methods)))
    for i, metric in enumerate(['accuracy', 'relevance', 'context_precision']):
        for j, method in enumerate(methods):
            heatmap_data[i, j] = processed_data[f"avg_{metric}_{method}"]
    
    # Đối với thời gian, hiển thị giá trị thực tế
    for j, method in enumerate(methods):
        heatmap_data[3, j] = processed_data[f"avg_time_{method}"]
    
    # Đối với token, hiển thị giá trị thực tế
    for j, method in enumerate(methods):
        heatmap_data[4, j] = processed_data[f"avg_tokens_{method}"]
    
    # Vẽ heatmap
    sns.heatmap(heatmap_data, annot=True, fmt=".2f", cmap="YlGnBu", 
              xticklabels=methods, yticklabels=metrics)
    
    plt.title('Tổng hợp kết quả các phương pháp RAG')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'combined', 'metrics_heatmap.png'), dpi=300)
    plt.close()
    ...
    bars1 = ax1.bar(methods, avg_accuracy, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    for bar in bars1:
        height = bar.get_height()
    ...
    bars2 = ax2.bar(methods, avg_relevance, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    for bar in bars2:
        height = bar.get_height()
    ...
    bars3 = ax3.bar(methods, avg_times, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    for bar in bars3:
        height = bar.get_height()
    ...
    bars4 = ax4.bar(methods, avg_tokens, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728'])
    for bar in bars4:
        height = bar.get_height()
    ...
    for i, method in enumerate(methods):
        ...
        # Vẽ đường
        ax5.plot(angles, values, linewidth=2, linestyle='solid', label=method)
...
def main():
    ...
    parser.add_argument(
        "--input-file", 
        type=str, 
        default="results/all_comparison_20250528_211152.json",
